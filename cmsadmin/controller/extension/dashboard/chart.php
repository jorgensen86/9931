...
class ControllerExtensionDashboardChart extends Controller {
	...
	public function chart() {
		...
		$json['customer']['backgroundColor'] = 'rgb(255, 60, 51)';
		$json['customer']['borderColor'] = 'rgb(255, 60, 51)';
		$json['order']['backgroundColor'] = 'rgb(255, 190, 51)';
		$json['order']['borderColor'] = 'rgb(255, 190, 51)';
		...
		switch ($range) {
			...
			case 'month':
				...
				for ($i = 1; $i <= date('t'); $i++) {
					...
					$json['xaxis'][] = date('j', strtotime($date));
				}
				break;
			case 'year':
