...
class ModelCheckoutOrder extends Model {
	...
	public function editOrder($order_id, $data) {
		...
		if ($data['preorder']) {
			$this->addOrderHistory($order_id, PREORDER_ID);
		} else {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', days_of_delivery = '" . (isset($product['days_of_delivery']) ? (int)$product['days_of_delivery'] : 0) . "'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				// jorgensen - reset negative quantities to zero
				$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = 0 WHERE quantity < 0 AND subtract = '1'");
			...
			if($order_status_id != PREORDER_ID) {
