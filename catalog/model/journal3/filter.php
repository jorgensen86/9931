...
class ModelJournal3Filter extends Model {
	...
	public function filterBase() {
		switch (Arr::get($this->request->get, 'route')) {
			/* Jorgensen enable filter to home page route */
			case '':
				return $this->url->link('common/home', '', true);
			case 'common/home':
				return $this->url->link('common/home', '', true);
			/* Jorgensen enable filter to home page route */
		...
		foreach ($this->request->get as $k => $v) {
			...
			switch ($k) {
				...
				case 'ft';
					...
					break;
				/* jorgensen - add global search to filters */
				case 'fs';
					$filter_data['filter_name'] = $v;
					break;
				/* jorgensen - add global search to filters */
				default:
					switch (substr($k, 0, 2)) {
	...
	public function getCategories($filter = null) {
		...
		";
		if(!is_null($filter)) {
			$sql .= " AND c.parent_id = '" . $this->db->escape($filter) . "'";
		} elseif (is_numeric($filter_category_id = Arr::get(static::$filter_data, 'filter_category_id'))) {
